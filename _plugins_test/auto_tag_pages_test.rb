...
#
# Unit tests for _plugins/auto_tag_pages.rb. Run with:
#
#   bundle exec ruby _plugins_test/auto_tag_pages_test.rb
#
# Same conventions as normalize_empty_slug_test.rb — kept outside _plugins/
# so Jekyll doesn't auto-load it. The plugin's Jekyll-integration path is
# guarded behind `defined?(Jekyll::Generator)` so loading without Jekyll
# only registers the pure `summarise` helper.

require_relative '../_plugins/auto_tag_pages'
...
run('curated tags never marked missing even when no post uses them') do
  ...
  posts = [['Python']]
  missing, _ = Jekyll::AutoTagPages.summarise(
    curated: curated, post_tag_lists: posts, slugify: SLUGIFY,
  )
  check(missing.empty?,
...
run('count reflects how many post tag-lists reference each name') do
  ...
  by_name = all.each_with_object({}) { |t, h| h[t['name']] = t['count'] }
...
run('url uses slugified name regardless of case/punctuation') do
  ...
  by_name = all.each_with_object({}) { |t, h| h[t['name']] = t['url'] }
