...
- **No consumer lint CI.** The heavyweight lint toolchain is platform-internal; in this consumer the pre-commit hook (`scripts/lint-staged.sh`) is the only lint backstop. Run the tools locally to debug a failure.

## Adding a new language / file type

1. Pick the best-in-class linter; add it to the right manifest (npm `package.json` for JS-runtime tools; pip for Python-family; binary download for compiled tools).
2. Create its config file (prefer a dedicated dotfile; only Python config lives in `pyproject.toml`).
3. Add a per-language branch to `scripts/lint-staged.sh` (tool-availability-gated so missing tools skip, never block).
4. Document the toolchain row + any rule relaxations in AGENTS.md → "Code quality".
