  ...
  test("brand-new contributor: create → fill → image → save → publish → verify", async ({
    ...
    await measure("01-open-admin-and-login", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "Open `/admin/`. The Decap login screen shows a single button. On production this hands off to GitHub OAuth; locally (here) `decap-server` skips auth so the test can drive the next steps directly.",
      });
    });
    ...
    await measure("02-navigate-new-post", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "Click **Posts** in the left sidebar, then **New Post**. The form mounts with empty fields for Title, URL Slug, Date, Excerpt, Body, Tags, Featured Image, Published, and Publish Date.",
      });
    });
    ...
    await measure("03-fill-title-slug-body", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "Type a Title (the page heading), set a URL Slug (the path under `/blog/`), and write the Body. The slug is the only field with a constraint — keep it lowercase + hyphens so the public URL stays clean.",
      });
    });
    ...
    await measure("04-add-featured-image", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "Click **Choose Image** on the Featured Image widget. The media library opens; either pick an existing upload or use the **Upload** control to add a new one. After choosing, the path lands in the field's text and a thumbnail renders below it.",
      });
    });
    ...
    await measure("08-fetch-public-url", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "After Save settles, the post is reachable at `/blog/<slug>/`. On production this happens once `deploy-production.yml` finishes its `aws s3 sync` and CloudFront invalidation, typically within ~2 minutes of the merge.",
      });
    });
    ...
    await measure("09-assert-featured-image", PER_STEP_BUDGET_MS, async () => {
      ...
      await captureStep(page, {
        ...
        body:
          "The post layout renders `<img class=\"featured-image\">` inside the `.post-header` block when `featured_image` is set. On production the same `<img>` resolves through CloudFront → S3, so the contributor sees the image they uploaded without any further action.",
      });
    });
