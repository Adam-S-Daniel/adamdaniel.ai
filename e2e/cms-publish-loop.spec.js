 ...
 */
async function writeCanaryViaPr({
  runId,
  bodyText,
  message,
  prTitle,
...
test.afterAll(async ({}, testInfo) => {
  ...
  console.warn(
    `[cleanup-harness] ${reason}; opening fixture PR to restore baseline`,
  );
...
test("@canary-readonly production canary URLs serve their baselines", async ({
  ...
  for (const c of CANARIES) {
    await test.step(`Fetch ${c.publicPath} and assert baseline`, async () => {
      ...
      expect(res.status(), `${c.publicPath} should return 200 from prod`).toBe(
        200,
      );
