...
test("CMS — tags lifecycle (host repo, target main)", async ({ page }, testInfo) => {
  ...
  // ── 3. Wait for the chain to land the description on /tags/<slug>/
  await test.step("Wait for /tags/<slug>/ to serve the description (chain complete)", async () => {
    await waitForChangeReflected({
      ...
      urlCheck: async () => {
        ...
        return (await res.text()).includes(TAG_DESCRIPTION);
      },
  ...
  // ── 5. Wait for the chain to drop the description from /tags/<slug>/
  await test.step("Wait for /tags/<slug>/ to drop the description (chain complete)", async () => {
    await waitForChangeReflected({
      page,
      pillId: PILL_PROD,
      urlCheck: async () => {
        ...
        //   - 404 (Jekyll didn't generate /tags/<slug>/ — no posts
        //     tagged it, so auto_tag_pages skipped it after the entry
        //     was deleted).
        //   - 200 without our description (some post still tags it,
        //     archive page renders, but the deleted Tags entry's
        //     description text is gone).
