...
function makeDom() {
  ...
  class El {
    ...
    set innerHTML(v) { this._html = String(v); this.children = []; bump(); }
    ...
    _qsa(sel) {
      ...
      m = /^a\[href\*=["'](.+)["']\]$/.exec(sel);
      if (m) return all.filter((e) => e.tagName === "A" && (e.getAttribute("href") || "").includes(m[1]));
      m = /^\[role=["'](\w+)["']\]$/.exec(sel);
  ...
  return { El, mutations: () => mutations, resetMutations: () => { mutations = 0; } };
}
...
function runEnhance(posts) {
  ...
  const sandbox = {
    ...
    JSON, Date, decodeURIComponent, encodeURIComponent, RegExp,
    Object, Array, String, Math, Promise,
  ...
  for (let i = 0; i < MAX; i++) {
    ...
    if (dom.mutations() === 0) { converged = true; break; }
  }
...
test.describe("posts-list-enhance.js — reorderFixturesLast fixed point", () => {
  for (const count of [1, 2, 3]) {
    test(`settles with ${count} fixture(s) present (no infinite reorder loop)`, () => {
      ...
      expect(cardCount, "entry cards must never be removed (augment, not replace)").toBe(posts.length);
    });
  }
