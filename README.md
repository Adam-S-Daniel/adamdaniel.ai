...
**Atom feeds.** The site exposes a global feed at `/feed.xml` (via `jekyll-feed`) plus per-tag feeds at `/tags/<slug>/feed.xml` (generated by the gem-delivered `tag_feeds` Jekyll plugin from cms-platform). The two share a single template (`_layouts/atom_feed.xml`) so readers parse both identically. The RSS icon partial (`_includes/feed-link.html`) is mounted on the site-wide layout and on each tag page.
