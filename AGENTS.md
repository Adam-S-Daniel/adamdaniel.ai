...
- **No back doors in setup or cleanup either.** "Never bypass the UI" applies to fixture setup and post-test cleanup, not just the spec body. Reading canary content from main via the GitHub Contents API or seeding a baseline through a labelled fixture PR (`cms-fixture-pr.js`'s `seedFixtureViaPr`/`removeFixtureViaPr`) skips the very Save → Status:Ready → Publish Now → deploy chain the test exists to validate. Setup should drive Decap UI to assert the baseline is in place (read from the entry editor's body field, compare to expected content), and cleanup should drive Decap UI to restore it (edit body → Save → wait for the chain). This roughly doubles spec runtime — every test runs the publish loop both forward (apply marker) and backward (remove marker) — but every chain is UI-driven, so a regression on either direction surfaces as a test failure rather than a silent revert via API. The route-mocked `publish-via-auto-merge-browser.spec.js` is still allowed to use the shim's programmatic `__callMerge` because that spec's entire reason for existing is the shim's internal contract, not the editor's experience.
...
| `cms-publish-loop-prod.yml` | `push` (main), `workflow_dispatch` | `paths` (positive, push to main) | `e2e/cms-publish-loop-prod-mutate.spec.js`, `e2e/{decap-pat,github-actions-poll,base}.js`, `_posts/2099-01-01-e2e-mutation-canary.md`, `admin/**`, `playwright.config.js`, `package*.json`, `_config.yml`, `_layouts/post.html`, the workflow itself. Runs **post-merge** (not per-PR): the spec drives a REAL prod mutation, so firing it on every concurrent PR raced the shared canary + the deploy-production queue and flaked. The actual mutation is gated by `vars.PROD_PLAYGROUND_MODE == 'true'` (sunset switch) |
...
2. Repo variable `PROD_PLAYGROUND_MODE` must equal `'true'` (set in repo Settings → Variables and secrets → Actions → Variables). Flipping it `false` instantly stops every run from mutating prod with no code change. The fixture file stays in-tree as documentation.
