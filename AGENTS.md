...
| `cms-media-roundtrip.yml` | `schedule`, `push` (main), `workflow_dispatch` | `paths` (positive, push to main) | Only the workflow's own file (`.github/workflows/cms-media-roundtrip.yml`). Prod (`cms-publish-loop-prod.yml`) owns the shared infra paths (`admin/**`, `playwright.config.js`, `package*.json`, `_config.yml`, `_layouts/post.html`) on push; this loop covers them via its own 15:00 UTC cron instead, keeping the three real-prod loops' push-trigger paths disjoint (no co-arrival eviction in the shared `prod-mutating-loop` concurrency lane — see #1892 below) |
...
| `cms-publish-loop-host.yml` | `schedule` (12:00 UTC daily), `push` (main), `workflow_dispatch` | `paths` (positive, push to main) | `cms-publish-loop-host.yml` itself, `_e2e/canary-post.md`, `_e2e/canary-page.md`, `_e2e/canary-project.md`, `_layouts/canary.html`, `_layouts/default.html` — narrowed to this loop's OWN canary surfaces only (#1892: it used to also list `admin/**`/`playwright.config.js`/`package*.json`/`_config.yml`, which overlapped `cms-publish-loop-prod.yml`'s push paths and caused co-arrival eviction in the shared `prod-mutating-loop` lane). Runs post-merge; recursion gated by the shared `recursion-gate` job |
...
| `cms-publish-loop-prod.yml` | `push` (main), `workflow_dispatch` | `paths` (positive, push to main) | `cms-publish-loop-prod.yml` itself, `admin/**`, `playwright.config.js`, `package.json`, `package-lock.json`, `_config.yml`, `_layouts/post.html`. Runs **post-merge** (not per-PR): the spec drives a REAL prod mutation, so firing it on every concurrent PR raced the shared canary + the deploy-production queue and flaked. Gated by repo var `PROD_PLAYGROUND_MODE == 'true'` |
...
platform's reusable `e2e-tests.yml@v0.1.52` via `workflow_call`. The reusable checks
