...
When all three pass, the spec runs against `https://adamdaniel.ai/admin/`: it CREATES a born-published ephemeral post via the Decap "+ New Post" UI (Title + URL Slug + Date `2099-12-31` + Body + Published ON), labels the cms PR `cms/ready` → auto-merge → `deploy-production.yml`, fetches `/blog/e2e-prod-mutate-<runId>/`, asserts the run-unique marker is live, then DELETEs the post via the Decap UI (delete PR also merged via `cms/ready`, **never** "Publish Now") and asserts the URL 404s. The `afterAll` is an existence-only delete of any leftover orphan (#1771 step 4).
