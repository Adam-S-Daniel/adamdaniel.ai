...
_Last regenerated: 2026-05-27T16:37:34.532Z_
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:26.139Z.</sub>
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:26.899Z.</sub>
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:27.457Z.</sub>

---

## Editing a post

Every Save lands on a new `cms/<timestamp>` branch and opens a PR — your
edit is a PR-backed draft, not a live commit. The PR triggers the standard
test matrix and a CloudFront-backed preview at
`preview-pr<N>.adamdaniel.ai`.
...
<sub>URL: [http://localhost:4000/admin/index-local#/collections/posts/entries/2026-05-12-introducing-gha-bench](http://localhost:4000/admin/index-local#/collections/posts/entries/2026-05-12-introducing-gha-bench)</sub>

<sub>Captured by `e2e/cms-smoke.spec.js` → _Posts edit form: every declared field renders with visible content_ on `chromium-desktop-3k` at 2026-05-27T16:37:33.607Z.</sub>
...
<sub>Captured by `e2e/cms-editorial-workflow.spec.js` → _opening an existing post renders all fields editable + Delete button enabled_ on `chromium-desktop-3k` at 2026-05-27T16:37:10.992Z.</sub>

---

## Marking ready and publishing

Setting the Status dropdown to **Ready** adds the `cms/ready` label, which
fires `auto-merge-when-ready` in `cms-editorial-workflow.yml`. The PR then
auto-merges as soon as the required checks (validate-content, e2e,
visual-regression / approve-regression) all go green.
...
<sub>Captured by `e2e/cms-editorial-workflow.spec.js` → _editing an existing post and saving creates a workflow draft_ on `chromium-desktop-3k` at 2026-05-27T16:37:16.329Z.</sub>
...
<sub>Captured by `e2e/cms-publish-flow.spec.js` → _create a post in Decap, rebuild, and assert /blog/<slug>/ renders it_ on `chromium-desktop-3k` at 2026-05-27T16:37:11.933Z.</sub>
...
<sub>Captured by `e2e/cms-publish-flow.spec.js` → _create a post in Decap, rebuild, and assert /blog/<slug>/ renders it_ on `chromium-desktop-3k` at 2026-05-27T16:37:12.288Z.</sub>
...
<sub>Captured by `e2e/cms-publish-flow.spec.js` → _create a post in Decap, rebuild, and assert /blog/<slug>/ renders it_ on `chromium-desktop-3k` at 2026-05-27T16:37:16.947Z.</sub>

---

## Verifying on the public site

A successful merge to `main` triggers `deploy-production.yml`, which
`aws s3 sync`s the build into the production bucket and invalidates
CloudFront. Your edit goes live within ~2 minutes of the merge.
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:28.201Z.</sub>
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:28.650Z.</sub>
...
<sub>Captured by `e2e/cms-smoke.spec.js` → _admin loads, logs in, creates a tag, saves it, deletes it_ on `chromium-desktop-3k` at 2026-05-27T16:37:29.336Z.</sub>
